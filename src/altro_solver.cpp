...
namespace altro
{
  ...
  ErrorCodes ALTROSolver::SetDimension(int num_states, int num_inputs, int k_start, int k_stop)
  {
    ...
    for (int k = k_start; k < k_stop; ++k)
    {
      ...
      if (err != ErrorCodes::NoError)
        return err; // TODO: reset to previous state on error?
                                          ...
                                          a_float c, int k_start, int k_stop)
  {
    ...
    for (int k = k_stop; k < k_stop; ++k)
    {
      ...
      if (n != num_states)
        return ErrorCodes::DimensionMismatch;
                                                ...
                                                LastIndexMode last_index) const
  {
    ...
    Serial.print(F("WARNING [ALTRO]: Stopping index "));
    Serial.print(k_stop);
    Serial.print(F(" not greater than starting index "));
    Serial.print(k_stop);
    Serial.print(F("                 To set a single index, set k_stop = k_start + 1."));
      // clang-format on
    }
    return ErrorCodes::NoError;
  }
  ...
  void ALTROSolver::PrintStateTrajectory() const
  {
    ...
    Serial.println(F("STATE TRAJECTORY:"));
    for (int k = 0; k <= GetHorizonLength(); ++k)
    {
      Serial.println(fmt::format(" x[{:03d}]: [{}]", k, solver_->data_[k].x_.transpose().eval()).c_str());
    }
  }
  ...
  void ALTROSolver::PrintInputTrajectory() const
  {
    ...
    Serial.println(F("INPUT TRAJECTORY:"));
    for (int k = 0; k < GetHorizonLength(); ++k)
    {
      Serial.println(fmt::format(" u[{:03d}]: [{}]", k, solver_->data_[k].u_.transpose().eval()).c_str());
    }
  }
